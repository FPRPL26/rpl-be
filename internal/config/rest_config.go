...
func NewRest() RestConfig {
	...
	var (
		...
		classRequestTransactionRepo            repository.ClassRequestTransactionRepository      = repository.NewClassRequestTransactionRepository(db)
		classRequestTutorApplicationRepository repository.ClassRequestTutorApplicationRepository = repository.NewClassRequestTutorApplicationRepository(db)
		transactionRepository                  repository.ClassTransactionRepository             = repository.NewClassTransactionRepository(db)
		barterSkillRepository                  repository.BarterSkillRepository                  = repository.NewBarterSkillRepository(db)
		...
		barterSkillService                  service.BarterSkillService                  = service.NewBarterSkillService(db, barterSkillRepository, barterTransactionRepository, tutorProfileRepository)
		...
		classRequestTransactionController      controller.ClassRequestTransactionController      = controller.NewClassRequestTransactionController(classRequestTransactionService)
		classRequestTutorApplicationController controller.ClassRequestTutorApplicationController = controller.NewClassRequestTutorApplicationController(classRequestTutorApplicationService)
		barterSkillController                  controller.BarterSkillController                  = controller.NewBarterSkillController(barterSkillService)
		skillController                        controller.SkillController                        = controller.NewSkillController(skillService)
		portofolioController                   controller.PortofolioController                   = controller.NewPortofolioController(portofolioService)
	...
	routes.ClassRequestTransaction(server, classRequestTransactionController, middleware)
	routes.ClassRequestTutorApplication(server, classRequestTutorApplicationController, middleware)
	routes.BarterSkill(server, barterSkillController, middleware)
	routes.Skill(server, skillController)
	routes.Portofolio(server, portofolioController, middleware)
